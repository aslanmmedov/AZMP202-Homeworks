...
let catsort = document.querySelector("#cat-selector")
let searchprod = document.querySelector(".search");
sortrate.addEventListener("click",rating)
catsort.addEventListener("change", function(event){
    let catprod = [];
...
searchprod.addEventListener("input",function(event){
    let titprod = [];
    products.forEach(title => {
        if(title.title.toLowerCase().includes(event.target.value.toLowerCase())){
            titprod.push(title);
            // console.log(title);
        }
    });
...
// let deletebtns = document.querySelector(".dlt-txt");

// deletebtns.addEventListener("click",function(){
//     this.closest("tr").remowe();
// }); //delete islemir(
function rating(){
    let sortedprod = [];
